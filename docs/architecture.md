...
`rag.enabled`일 때 질문 분기 → 승인 자료/개인 기억 검색 → 출처·충돌·예산 선별 →
기존 LLM → 원문 충실도 검사 → 기존 final/speech 출력으로 연결합니다.
RAG 저장소는 기존 console 기억 DB와 분리하며 기본 비활성입니다.
시청자 저장/조회/공개 동의와 운영자 후보 승인을 모두 확인합니다.
상세 책임·삭제 원장·컨텍스트 무효화·진단·제한은 [LLM·RAG 계획](llm-rag-plan.md)을 따릅니다.
아래의 “종료하면 DB를 만들지 않음” 등은 RAG와 기존 영속 기억을 켜지 않은 기본 경로 설명입니다.
