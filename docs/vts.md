...
## 실제 모델 검증 준비 확인 (2026-09-23)

기준 `04564f1`에서 VTS 클라이언트·CLI·표정 연결 자동 테스트 98개를 통과했다. 현재 VTS 실행 프로세스와 포트 8001 리스너는 확인되지 않았고 실제 인증·모델·표정 검증은 미실행이다. 설치 전체를 조사한 것은 아니므로 미설치를 단정하지 않는다. [실제 모델 검증 체크리스트](vts-model-checklist.md)에 준비 항목과 인증 거부·토큰 폐기·모델 미로드/변경·VTS 종료·수동 복구 시험을 정리했다. 실제 검증 전 병합 보류를 유지한다.
