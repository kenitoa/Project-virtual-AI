...
기준 `04564f1`에서 고정 문장 WAV의 실제 Realtek 출력 재생, 독립 CLI `/stop` 후 다음 재생, 재생 중 `/quit` 후 새 프로세스 재생을 확인했다. 모두 종료 코드 0이다. 사용자는 지금 청취할 수 없다고 답했으므로 음질·실제 무음·청취 복구는 미확인이다. 실행 조건과 나중에 수행할 절차는 [별도 청취 체크리스트](voice-listening-checklist.md)에 기록했다. API 성공을 청취 완료로 판정하지 않는다.
