...
v1.0의 완료 범위와 진행 순서는 [v1.0 완료 기준](docs/v1-completion.md)을 우선한다(2026-09-23 확정). SQLite 기억과 마이크 STT는 각각 끌 수 있는 v1.0 필수 기능이며, 벡터 DB·모델 미세조정·다중 방송 플랫폼·게임 자동 조작·무인 24시간 방송은 후속 확장이다.

아래 표는 초기 개발 단계 구분이며 구현 완료 상태나 현재 진행 단계를 뜻하지 않는다. 음성·캐릭터·방송 플랫폼을 동시에 연결하지 않고 위 문서의 검증 순서에 따라 진행한다.
...
| 5. 기억·입력 및 품질 | SQLite 장기 기억, STT, 성능 검증; 세밀한 감정 표현은 후속 확장 | 기억·STT를 각각 켜고 끌 수 있고 기존 기능 테스트가 유지됨 |
